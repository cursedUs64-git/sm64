...
s16 gLllOctagonalMeshAction0[] = {
    ...
    LLL_OCTMESH_LINEAR_MOVE, 30,  0x4000, 0,   LLL_OCTMESH_CHANGE_DIR,  220, 900,     30,
    LLL_OCTMESH_CHANGE_DIR,  30,  0,      -30, LLL_OCTMESH_LINEAR_MOVE, 30,  -0x4000, 0,
    LLL_OCTMESH_CHANGE_DIR,  220, 900,    30,  LLL_OCTMESH_CHANGE_DIR,  30,  0,       -30,
    LLL_OCTMESH_RESET,       0
};
s16 gLllOctagonalMeshAction1[] = {
    // instruction              time moveAngle  forwardVel
    LLL_OCTMESH_WAIT_FOR_MARIO, 0,   0,   0,   LLL_OCTMESH_CHANGE_DIR,  475, 900,    30,
    LLL_OCTMESH_CHANGE_DIR,     30,  0,   -30, LLL_OCTMESH_LINEAR_MOVE, 30,  0x8000, 0,
    LLL_OCTMESH_CHANGE_DIR,     475, 900, 30,  LLL_OCTMESH_CHANGE_DIR,  30,  0,      -30,
    LLL_OCTMESH_RESET,          0
};

// picked by oBhvParams2ndByte
...
s32 lll_octagonal_mesh_move(s16 *actionTable, s32 actionOffset) {
    switch (actionTable[actionOffset]) {
        ...
        case LLL_OCTMESH_CHANGE_DIR:
            approach_f32_signed(&o->oForwardVel, actionTable[actionOffset + 2] / 100.0f,
                                actionTable[actionOffset + 3] / 100.0f);
            if (o->oTimer > actionTable[actionOffset + 1]) {
                actionOffset += 4;
                o->oTimer = 0;
            }
...
s32 lll_octagonal_mesh_find_y_offset(s32 *standTimer, f32 *posOffset, s32 standTimerInc,
                                     s32 moveDownAmount) {
...
void bhv_lll_moving_octagonal_mesh_platform_loop(void) {
    if (o->oAction == 0) {
        ...
        o->oLllOctmeshActionOffset = lll_octagonal_mesh_move(
            gLllOctagonalMeshActionList[o->oBhvParams2ndByte], o->oLllOctmeshActionOffset);
    ...
    if (lll_octagonal_mesh_find_y_offset(&o->oLllOctmeshStandTimer, &o->oLllOctmeshWaveOffset2, 0x400,
                                         -80)) {
        // nope, wave as usual
